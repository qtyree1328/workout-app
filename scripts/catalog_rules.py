...
def enrich(data,metadata):
    ...
    for e in data['exercises']:
        ...
        if e['variants'][0].get('type') in ['image','instruction']:m['dose']['cue']=e['variants'][0]['note']
        aliases=t['aliases']+[v['label'] for v in e['variants']]
        equipmentAliases={'None':['bodyweight','no equipment'],'Dowel':['bar','5 ft bar','stick','mobility bar'],'Resistance band':['bands','elastic','resistance'],'Yoga block':['block'],'Yoga blocks':['block'],'Band + block':['bands','block'],'Pull-up bar':['pullup','hanging','climbing','grip'],'Chair':['chair','supported'],'Stable support':['chair','supported'],'Wall':['wall'],'Light weights':['dumbbell','bottle']}
        m['tags']=sorted(set(t['regions']+[t['pattern'],training,e['kind'].lower(),e['equipment'].lower(),m['target'].lower(),m['difficulty'].lower(),m['strain'].lower()]+aliases+equipmentAliases.get(e['equipment'],[])))
        if 'upper back' in t['regions'] or 'lower back' in t['regions']:m['tags'].append('back')
        m['equipment']=[x for x in {'None':[],'Dowel':['bar'],'Resistance band':['band'],'Yoga block':['block'],'Yoga blocks':['block'],'Band + block':['band','block'],'Pull-up bar':['pull-up bar'],'Chair':['chair'],'Stable support':['chair'],'Wall':['wall'],'Light weights':['weights'],'Bench':['bench'],'Band + bench':['band','bench'],'Weighted dowel':['weighted bar']}.get(e['equipment'],[e['equipment'].lower()])]
    return metadata
...
def build_groups(meta):
    groups=[]
    def step(id,phase='main',side='Both',work=None,rest=None,setno=1):
        m=meta[id]
        return dict(exercise=id,mode='time',work=m['work'] if work is None else work,reps=m['reps'],rest=m['rest'] if rest is None else rest,side=side,variant=0,phase=phase,set=setno,cue=m['dose']['cue'],repTarget=m['reps'] if m['trainingType']=='strength' else None)
    ...
    def emit(id,title,focus,steps,kind,rationale,nominal):
        steps[-1]['rest']=0
        total=seconds(steps)
        ...
        groups.append(dict(id=id,name=title,description=f'{kind} · {nominal}-minute option',steps=steps,builtin=True,kind=kind,focus=focus,tags=tags,equipment=equipment,durationSeconds=total,budgetMinutes=nominal,rationale=rationale,coverExercises=list(dict.fromkeys(s['exercise'] for s in active if s['phase']=='main'))[:3]))
    mobility={
