...
def render(password):
    ...
    for e in entries():
        page = doc[e['variants'][0]['page'] - 1]
        r = page.rect
        full = pymupdf.Rect(0, 0, r.width, r.height * 0.9)            # drop the footer
        thumb = pymupdf.Rect(0, r.height * 0.12, r.width * 0.6, r.height * 0.88)  # main pose only
        page.get_pixmap(matrix=pymupdf.Matrix(1400 / r.width, 1400 / r.width), clip=full).save(ROOT / e['variants'][0]['image'], jpg_quality=78)
        page.get_pixmap(matrix=pymupdf.Matrix(480 / thumb.width, 480 / thumb.width), clip=thumb).save(ROOT / e['variants'][0]['thumbnail'], jpg_quality=78)
